...
export default function MiniGame() {
  ...
  const [koala, setKoala] = useState<Pet>({
    name: 'Koala', emoji: '🐨', hunger: 70, happiness: 80, energy: 90, animation: ''
  })
  const [frog, setFrog] = useState<Pet>({
    name: 'Kurbağa', emoji: '🐸', hunger: 60, happiness: 75, energy: 85, animation: ''
  })
  ...
  const animate = (setPet: typeof setKoala, anim: string) => {
    setPet(p => ({ ...p, animation: anim }))
    setTimeout(() => setPet(p => ({ ...p, animation: '' })), 600)
  }

  const getMood = (pet: Pet) => {
  ...
  const petAction = (setPet: typeof setKoala, pet: Pet, e: React.MouseEvent) => {
    ...
    animate(setPet, 'wiggle')
    setPet(p => ({
      ...p,
      happiness: Math.min(100, p.happiness + 15),
      energy: Math.min(100, p.energy + 5),
    }))
  ...
  const sleep = (setPet: typeof setKoala, pet: Pet) => {
    ...
    setPet(p => ({
      ...p,
      energy: Math.min(100, p.energy + 40),
  ...
  const PetCard = ({ petData, setPet }: { petData: Pet; setPet: typeof setKoala }) => {
    ...
    return (
      <div style={{ flex: 1, background: 'rgba(253,246,236,0.8)', borderRadius: '16px', border: '1px solid rgba(201,168,76,0.2)', padding: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <div style={{ textAlign: 'center' }}>
          <div
            onClick={(e) => petAction(setPet, petData, e)}
            style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>🍽 Açlık</span>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>{Math.round(petData.hunger)}%</span>
          </div>
          <Bar value={petData.hunger} color="#e8a0a0" />
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>♡ Mutluluk</span>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>{Math.round(petData.happiness)}%</span>
          </div>
          <Bar value={petData.happiness} color="#c9a84c" />
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>⚡ Enerji</span>
            <span style={{ fontFamily: 'Lato', fontSize: '0.65rem', color: '#a07070' }}>{Math.round(petData.energy)}%</span>
          </div>
          <Bar value={petData.energy} color="#90c890" />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
          {[
            ...
            { label: '💤 Uyu', action: () => sleep(setPet, petData), color: '#b0c8e8' },
          ].map(btn => (
            <button
              key={btn.label}
              onClick={btn.action}
  ...
  return (
    <>
      ...
      >
        ...
        {!open && (
          <span style={{
            writingMode: 'vertical-rl', fontSize: '0.65rem',
            color: '#c9a84c', fontFamily: 'Lato', fontWeight: 700, letterSpacing: '0.1em',
          }}>OYUN</span>
        )}
      </button>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px', overflowY: 'auto', scrollbarWidth: 'none' as const, flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <PetCard petData={koala} setPet={setKoala} />
            <PetCard petData={frog} setPet={setFrog} />
          </div>

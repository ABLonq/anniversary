...
export default function CardItem({ card, onDelete, onEdit, onClick }: Props) {
      ...
      <div style={{ position: 'relative', height: '200px', overflow: 'hidden' }}>
        {images.map((src, i) => (
          <img key={i} src={src} alt={card.title} style={{
            ...
            objectFit: 'cover', opacity: i === current ? 1 : 0, transition: 'opacity 0.8s ease',
          }} />
        ))}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(90,62,62,0.4) 0%, transparent 50%)' }} />
        ...
        <div style={{
          ...
          background: 'rgba(253,246,236,0.9)', borderRadius: '20px', padding: '3px 10px',
          display: 'flex', alignItems: 'center', gap: '4px',
        }}>
          <span style={{ color: '#c9a84c', fontSize: '0.6rem' }}>✦</span>
          <span className="font-body" style={{ color: '#5a3e3e', fontSize: '0.7rem', letterSpacing: '0.05em' }}>{card.date}</span>
        </div>
      </div>
        ...
        <div className="flex justify-end gap-2 mt-3">
          <button
            ...
            style={{
              ...
              padding: '4px 8px', borderRadius: '6px', transition: 'opacity 0.2s',
            }}

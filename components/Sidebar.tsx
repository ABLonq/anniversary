  ...
  return (
    <>
      <aside style={{
        ...
        borderRight: '1px solid rgba(201, 168, 76, 0.2)', zIndex: 30,
        ...
        <div style={{ padding: '2rem 1.5rem 1rem', borderBottom: '1px solid rgba(201, 168, 76, 0.15)' }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => toggle(false)} style={{ background: 'transparent', border: 'none', color: '#c9a0a0', cursor: 'pointer', fontSize: '1rem', padding: '4px' }}>✕</button>
          </div>
        </div>
        ...
        <nav style={{ padding: '1rem 0.75rem', flex: 1, overflowY: 'auto', scrollbarWidth: 'none' as const }}>
          {CATEGORIES.map((cat) => {
            ...
            const count = cat.id === 'tumü' ? Object.values(counts).reduce((a, b) => a + b, 0) : (counts[cat.id] || 0)
            return (
              <button key={cat.id}
                onClick={() => { onChange(cat.id); if (window.innerWidth < 1024) toggle(false) }}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '12px 14px', marginBottom: '4px', borderRadius: '14px', border: 'none', cursor: 'pointer',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                  ...
                  <span style={{ fontFamily: 'Lato', fontSize: '0.85rem', fontWeight: isActive ? 700 : 400, color: isActive ? '#5a3e3e' : '#8a6060', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{cat.label}</span>
                </div>
                <span style={{ fontFamily: 'Lato', fontSize: '0.7rem', fontWeight: 700, color: isActive ? '#c9a84c' : '#c9a0a0', background: isActive ? 'rgba(201,168,76,0.12)' : 'rgba(201,168,76,0.06)', borderRadius: '10px', padding: '2px 8px', flexShrink: 0 }}>{count}</span>
              </button>
            )
          })}
        </nav>
      ...
      {!expanded && (
        <button onClick={() => toggle(true)} style={{
          ...
          background: 'rgba(253, 246, 236, 0.95)', border: '1px solid rgba(201, 168, 76, 0.3)',
          borderLeft: 'none', borderRadius: '0 12px 12px 0', padding: '16px 10px',
          color: '#c9a84c', cursor: 'pointer', fontSize: '0.8rem', zIndex: 30,
          boxShadow: '4px 0 20px rgba(201, 168, 76, 0.1)', writingMode: 'vertical-rl',
          letterSpacing: '0.1em', fontFamily: 'Lato',
        }}>KATEGORİLER</button>
      )}
    </>
  )
